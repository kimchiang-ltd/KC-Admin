// ============================================================
// VatPurchaseBlankForm (#348) — รายงานภาษีซื้อ (แบบฟอร์มเปล่า)
// ============================================================
// Clone of VatSalesBlankForm (#342b) for purchase side.
// Same layout/behavior, different labels (ผู้ขาย instead of ผู้ซื้อ),
// saves as type="VAT_PURCHASE" in Other Docs sheet.
...
import { CustomerAutocomplete } from "../../shared/autocomplete.jsx";

import { api } from "../../shared/api.jsx";
import { fmtAmt } from "../../shared/utils.jsx";
import { prefetchVatConfig, getVatConfigCache, makeDocNo } from "./VatSalesBlankForm.jsx";
...
// ── #356: supplier list for VAT Purchase (separate from customer list) ──
const supplierApi = { getCustomers: () => api.getSuppliers("") };

// ── A4 Print ─────────────────────────────────────────────
function printA4Purchase(rows, monthIdx, year, company) {
...
<div style="text-align:center;margin-bottom:8px;">
  <div style="font-size:15pt;font-weight:700;">รายงานภาษีซื้อ</div>
...
<table style="font-size:9pt;">
  <thead>
    <tr>
      ...
      <th rowspan="2" style="border:1px solid #333;padding:3px 2px;text-align:center;vertical-align:middle;width:25%;">ชื่อผู้ขายสินค้า<br>ผู้ให้บริการ</th>
      <th rowspan="2" style="border:1px solid #333;padding:3px 2px;text-align:center;vertical-align:middle;width:16%;font-size:8pt;line-height:1.3;">เลขประจำตัวผู้เสีย<br>ภาษีอากรของผู้ขาย<br>สินค้าผู้ให้บริการ</th>
...
// ── Main Component ───────────────────────────────────────
function VatPurchaseBlankForm({ rowId: initialRowId, onBack }) {
  const now = new Date();
  ...
  const [rowId, setRowId]     = useState(initialRowId || null);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        // Config: use shared cache (prefetched by list page), or fetch if not yet cached
        const cached = getVatConfigCache();
        const cfg = cached || await prefetchVatConfig();
        if (cfg) {
  ...
  // #357 — track loaded suppliers for new-supplier detection on blur
  ...
  // #363 — styled modal for new supplier prompt (replaces window.confirm)
  ...
  const checkNewSupplier = useCallback((id) => {
    ...
    setTimeout(() => {
      ...
      const exists = suppliersRef.current.some(s => s.name.toLowerCase() === name.toLowerCase());
      if (exists) return;
  ...
  const handleContactConfirm = () => {
    ...
    api.createSupplier({ name, taxId, branch }).then(() => {
      api.getSuppliers("").then(list => { suppliersRef.current = list; });
    }).catch(e => setActionError("เพิ่มผู้ขายไม่สำเร็จ: " + (e.message || e)));
  ...
  const handlePrint = () => {
    ...
    printA4Purchase(rows, month, year, company);
  };
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await api.deleteOtherDoc(rowId, "VAT_PURCHASE");
      // Brief success message then return to list
      setDeleting(false);
      setSaved(false);
      onBack();
    } catch (e) {
  ...
  return (
    <div style={{ position: "relative" }}>
      {/* #363 — styled new-supplier modal */}
      {newContactPrompt && <ConfirmModal
        message={`ไม่พบ "${newContactPrompt.name}" ในรายชื่อผู้ขาย\nต้องการเพิ่มเป็นผู้ขายใหม่ไหม?`}
        confirmLabel="ใช่ เพิ่มผู้ขาย"
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead>
            <tr style={{ background: "#f8f8f8" }}>
              ...
              <th style={{ padding: "8px 4px", textAlign: "left", border: `0.5px solid ${C.border}`, fontWeight: 500, minWidth: 160 }}>ชื่อผู้ขายสินค้า/ผู้ให้บริการ</th>
              ...
              <tr key={r.id} style={{ background: i % 2 === 0 ? "white" : "#fafafa" }}>
                ...
                <td style={{ padding: "3px 4px", border: `0.5px solid ${C.border}` }}>
                  <input value={r.tiNo} onChange={e => updateRow(r.id, "tiNo", e.target.value)}
                    placeholder="เลขที่" style={{ ...cellInput, width: "100%" }} />
                </td>
                <td style={{ padding: "3px 4px", border: `0.5px solid ${C.border}` }}>
                  <CustomerAutocomplete
                    ...
                    onCustomersLoaded={list => { suppliersRef.current = list; }}
                    apiOverride={supplierApi}
                ...
                <td style={{ padding: "3px 4px", border: `0.5px solid ${C.border}`, textAlign: "center" }}>
                  <input type="checkbox" checked={r.branchHq}
                    onChange={e => { updateRow(r.id, "branchHq", e.target.checked); setTimeout(() => checkNewSupplier(r.id), 0); }}
                    style={{ cursor: "pointer" }} />
                </td>
                <td style={{ padding: "3px 4px", border: `0.5px solid ${C.border}` }}>
                  <input value={r.branch} onChange={e => updateRow(r.id, "branch", e.target.value.replace(/\D/g, "").slice(0, 5))}
                    onBlur={() => checkNewSupplier(r.id)}
...
// ── #348 VAT Purchase List (same pattern as VatSalesListPage) ──
let _vatPurchaseCache = null;

function VatPurchaseListPage({ onViewChange }) {
  const [docs, setDocs]       = useState(_vatPurchaseCache || []);
  const [loading, setLoading] = useState(!_vatPurchaseCache);
  const [error, setError]     = useState("");
  ...
  const loadList = async (silent) => {
    ...
    setError("");
    try {
      const res = await api.listOtherDocs("VAT_PURCHASE");
      const rows = res?.rows || [];
      _vatPurchaseCache = rows;
      setDocs(rows);
    } catch (e) { if (!silent) setError(e.message || "โหลดไม่สำเร็จ"); }
    finally { setLoading(false); }
  };

  useEffect(() => { loadList(_vatPurchaseCache ? true : false); prefetchVatConfig(); }, []);

  // #350 — update breadcrumb when entering/leaving form view
  const handleCreate = () => { setEditId(null); setView("form"); if (onViewChange) onViewChange("สร้างรายงานภาษีซื้อ"); };
  const handleEdit   = (id) => { setEditId(id); setView("form"); if (onViewChange) onViewChange("แก้ไขรายงานภาษีซื้อ"); };
  const handleBack   = () => { setView("list"); setEditId(null); loadList(true); if (onViewChange) onViewChange("รายงานภาษีซื้อ"); };
  ...
  if (view === "form") return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 500 }}>{editId ? "แก้ไขรายงานภาษีซื้อ" : "สร้างรายงานภาษีซื้อ"}</span>
      </div>
      <VatPurchaseBlankForm rowId={editId} onBack={handleBack} />
    </div>
  );
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        <div style={{ fontSize: 16, fontWeight: 500, flex: 1 }}><FileText size={15}/> รายงานภาษีซื้อ</div>
        <Btn primary onClick={handleCreate}>+ สร้างรายงานภาษีซื้อใหม่</Btn>
      </div>
            ...
            {docs.map((d, i) => {
              ...
              return (
                <tr key={d.id} style={{ cursor: "pointer", background: i % 2 === 0 ? "white" : "#fafafa" }}
                  ...
                  <td style={{ padding: "8px 6px", border: `0.5px solid ${C.border}`, fontWeight: 500, color: C.accent }}>{d.docNo || makeDocNo("VAT_PURCHASE", d.subject, docs.slice(i + 1).map(x => x.docNo || ""))}</td>
...
export { VatPurchaseBlankForm, VatPurchaseListPage };
