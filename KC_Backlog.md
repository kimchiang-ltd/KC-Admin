...
| 102 | BN Create — PDF format toggle wording | ⏳ Pending | D: รูปแบบ PDF แนวตั้ง/แนวนอน wording is unclear. Suggest reframing as pages-per-paper (2 หน้า/แผ่น vs 1/แผ่น). Reconcile with naming used elsewhere (print vs pdf) for consistency. | P4xS | 2026-06-19 |
...
| 115 | BN Create — edit customer info (name/address/phone) during creation | ⏳ Pending | D: During BN creation the right panel shows the customer but their name/address/phone can't be edited inline. Request: allow editing customer info at BN-create time (affects the BN PDF; decide whether it also updates the Customers sheet or is per-BN override). Scope TBD. | P3xM | 2026-06-19 |
...
| 111 | Refactor DN/TI forms — shared item editor + form hook | ⏳ Pending | D: DeliveryNoteForm (254) + TaxInvoiceForm (293) duplicate item grid, updateItem/cellInput, addRow/addContinuationRow, collapseItems, DESC_MAX/DETAIL_MAX, descWidth, similar-name warning, handleSave. Extract <InvoiceItemsEditor> + useInvoiceForm. ~150–200 lines saved. Supersedes/absorbs #78. HIGH value, HIGH risk — core forms, needs thorough test. | P3xL | 2026-06-19 |
...
| **UI** | | | | | |
| 34 | DETAIL_MAX frontend cap | ✅ Done | D: Hard character limit on detail field per segment (like DESC_MAX); prevent overflow in PDF right block. **Done v1.4.19**: DETAIL_MAX=32 added; applied in both DN and TI `updateDetailItem` guards. Value 32 is a starting point — calibrate after test print if real Thai data hits the cap. | P1xS | 2026-06-13 |
